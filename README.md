...
Hjem owns per-user files and packages. Hjem feature modules are imported globally because its extraModules scope is global; typed profile membership gates each feature per user. NixOS desktop modules provide system integration only, with typed private `nixComputers.desktop` overrides for cross-file settings.
